...
<div id="searchframe">
<div class="searchlist" ><c:forEach var="topic" items="${searchResult.topicFilters}">
...
<p id="search-list05">
   <c:forEach var="singleResult" items="${tResult.result}" varStatus="status">
   <c:if test="${status.index % 7 == 0}"><p class="listname"></c:if>
   ...
   </c:if>><c:out value="${singleResult.name}" /></a><c:if test="${(status.index % 7 == 6) || (status.last)}"></p></c:if></c:forEach>
</p>
<!-- keyword pop dialog -->
...
<span><a class="comeback01" href="javascript:gA('TopicResult')">&lt;&lt;返回</a></span>
</div>
<div class="tooltip05"></div>
</div>
